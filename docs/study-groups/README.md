...
客户端、服务端、数据库与结算已实现；2026-09-09 小组后端部署生产，2026-09-10 统一资料服务部署，客户端已有开发版上传，尚未提交审核或正式发布。真实微信账号间的完整联调与发布门禁仍待执行；下方保留各阶段验证记录。
...
- 当前小组统一展示用户保存的个人头像昵称；旧组内称呼与内置头像继续兼容未设置资料的账户。退出后不再向原组展示个人头像。
...
## 2026-09-10 小组头像与用语调整（服务端已部署，客户端待上传）

用户界面将「契约」改为「小组规则」，成员契约改为成员资料，历史改为小组记录；保留既有协议字段、错误码、结算和数据标识。

新增 `POST /v1/groups/avatar`，通过现有认证请求提交 requestId、membershipId 和 image（PNG/JPEG/WebP base64 data URI，最多128KB）。image为空字符串则恢复默认。只允许修改本人的当前成员记录，头像仅在同组鉴权响应内返回，不提供公开图片URL，邀请码预览不返回头像。响应 `avatarEnabled:true` 表示当前后端支持；旧服务端保持默认头像，不展示不可用的头像入口。

增量迁移v2创建 `group_avatars` 表；个人头像压缩到160×160后上传，个人资料保存和小组页加载时，在当前协议授权有效且后端支持的前提下自动同步。重复刷新对比图像内容，已同步则不重复写入；失败保留原头像，提示下拉刷新重试。更新不覆盖本机个人资料。退出、系统离席删除本轮头像，重新加入自动同步；历史不显示已结束成员头像。请求去重表仅存指纹与成功结果，不保存图像副本。

2026-09-10 已按用户授权部署服务端，批次 `20260910T012548Z`，备份和验证见 server/README.md 的当日部署记录；尚未重新上传微信版本。同步更新微信后台隐私指引中的小组头像用途，协议版本更新为2026-09-10-v2。

## 2026-09-10 后续资料与周表优化（已部署并上传开发版）

用户进一步授权统一个人昵称并扩展服务端。新增独立 user_profiles 和 /v1/profile，当前小组读取统一头像昵称，原组内称呼仅兼容旧资料；不改变状态机。新资料引导与周卡问号弹层、图像参考、规格和真实截图见 [本轮交付](../designs/profile-weekly/README.md)。统一资料服务已部署批次20260910T050633Z，客户端1.0.1已于13:12上传微信开发体验版，未提交审核或正式发布；完整测试176/176通过，部署和未完成真机验收见上述交付记录。
