...
Page({
  ...
  async load() {
    ...
    try {
      ...
      this.setData({ result: { ...d, summary: view.summary(d.summary, d.group), members: d.members.map(m => ({ ...m, avatarPath: m.avatarImage || '/assets/groups/avatar-' + m.avatar + '.svg', rateText: m.rate === null ? '—' : m.rate + '%', statusText: m.status === 'formal' ? '正式席' : m.status === 'observer' ? '旁听席' : '已离席' })) },
