...
Page({
  async onShow() {
    try { await require('../../utils/profileSync').restore(); } catch (e) { this.setData({ error: api.errorText(e) }); }
    const p = require('../../utils/profile').getProfile(); this.setData({ nickname: p.customized ? p.nickname : '' });
  },
  ...
  async onSubmit() {
    ...
    if (!d.name.trim() || !d.studyTopic.trim() || !d.nickname.trim()) { this.setData({ error: '请填写小组名称、学习内容，并设置个人昵称。' }); return; }
    ...
    if (!d.accepted) { this.setData({ error: '请先阅读并接受小组规则。' }); return; }
