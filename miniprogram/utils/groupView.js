...
function member(value) {
  return { ...value, avatarPath: value.avatarImage || '/assets/groups/avatar-' + value.avatar + '.svg', today: status(value.today),
...
function dashboard(data) {
  ...
  if (me.today.state === 'joined') { title = me.status === 'observer' ? '今日加入 · 旁听席' : '今日加入'; hint = me.status === 'observer' ? '原旁听状态已保留，回归挑战从明天重新开始。' : '从明天起按小组规则考核。'; }
