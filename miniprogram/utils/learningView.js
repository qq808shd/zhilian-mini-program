...
function getLearningOverview() {
  ...
  const topicViews = topics.map((topic) => {
    ...
    const learnedCount = sets.reduce((sum, set) => {
    ...
    return { ...topic, ...stats, knowledgeCount, learnedCount, setCount: sets.length,
      progress: knowledgeCount ? Math.round(learnedCount / knowledgeCount * 100) : 0,
      meta: `已浏览 ${learnedCount} / ${knowledgeCount} 条 · ${sets.length} 组`,
      badge: stats.activeWrongCount ? `${stats.activeWrongCount} 道待复习 · 正确率 ${stats.accuracy}%` : stats.attempts ? `累计正确率 ${stats.accuracy}%` : "尚未练习" };
  });
  const positions = wx.getStorageSync(POSITION_KEY) || {};
  const recent = Object.values({ ...progress, ...positions }).sort((a, b) => b.updatedAt - a.updatedAt).map((saved) => {
    ...
    return { key: `${topic.id}_${set.index}`, topicId: topic.id, topicName: topic.name, moduleName: getModuleById(topic.moduleId).name,
      ...
      title: set.items[index].title, progress: Math.round((index + 1) / set.count * 100), updatedAt: saved.updatedAt };
  ...
  return { topics: topicViews, recent, weakTopics, summary: summarize(records) };
}
