...
准备提供给朋友使用和提交审核前，还要按实际数据处理情况核对微信公众平台中的“小程序用户隐私保护指引”，说明用户标识和学习记录用于登录识别、跨设备同步及错题复习；保存后的个人头像昵称通过统一资料接口绑定账户并向当前同组成员展示，小组另存手动记录和参与状态。不收集手机号。
...
- 个人昵称不上传；学习小组保存组内称呼，加入后自动同步个人头像压缩副本供同组查看，未设置时使用内置头像，不收集手机号。
...
部署批次 `20260910T012548Z`，北京时间 09:27:28 启动新服务。已部署自动同步头像接口、增量迁移 v2 与相关共享规则；客户端尚未重新上传微信平台。

本地完整测试 170/170 通过；服务器候选测试共 31/31 通过（初次遗漏 group-fixture.js，补齐后小组 21 项重测通过，原 API 10 项通过结果有效）。生产 14 个运行文件与候选 SHA-256 清单一致，systemd active/running、NRestarts=0，公网 HTTPS /health 正常，匿名 POST /v1/groups/avatar 返回 401。未使用真实用户上传头像，实际微信头像自动同步仍待新版客户端验收。

服务器备份 `/var/backups/zhilian-api/20260910T012548Z/` 保留一致 SQLite 快照、旧 src、三个旧共享模型和部署清单，未下载到本机。迁移前后核对原 12 张表，业务行内容全部不变；迁移表仅新增版本 2，group_avatars 初始为空，数据库完整性及外键检查通过。

回滚：停服务、留存当前代码，恢复上述备份的 src 以及 learningModel.js、memoryScheduler.js、groupRules.js，再启动服务。保留迁移后的数据库，不用部署前快照覆盖上线后的新记录。

### 2026-09-10 统一个人资料部署记录

新增认证 GET/PUT `/v1/profile`，`user_profiles` 为 additive 新表（user_id、nickname、image、revision、updated_at），`/health` 增加 profileVersion=1。原 OpenID/userId/session 与学习同步版本不变。头像128KB上限、昵称1–24字、乐观版本冲突和相同内容幂等重试；当前小组展示读取统一资料，老资料回退原成员称呼/头像。账户资料在离席后保留，但离席头像不再对原组展示。

本轮已部署批次`20260910T050633Z`，包含新增profiles.js与相关api/database/groups/service及三个共享模型；微信开发体验版1.0.1已上传，未提交审核或正式发布。微信平台头像昵称用途仍需与2026-09-10-v3协议对应更新。[规格与验证](../docs/designs/profile-weekly/README.md)。

本轮本地全套测试176/176，服务器候选测试33/33通过。生产15个运行文件SHA-256逐一一致；systemd active/running、NRestarts=0，公网HTTPS /health包含profileVersion=1，匿名GET /v1/profile返回401。备份`/var/backups/zhilian-api/20260910T050633Z/`包含一致SQLite快照、旧src、三个旧共享模型及部署清单，未下载到本机。迁移前后原13张表全部行摘要相同，新增user_profiles初始为空，完整性及外键检查通过。回滚仅按上一节方法恢复此批次旧代码和共享模型，保留新数据库及后续用户记录。
