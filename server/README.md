...
> 今日学习设置协议 2：preferences 保存分类与 1–200 的整数新学数量（version: 2、batchId 为空），客户端按分类实际总量进一步限制；来源批次不再是设置。响应 learningSettingsVersion: 2 后客户端才确认设置及对应计划。基础计划最多 215 项（复习 10、新学 200、配套题 5），含本轮重练最多 225 项。仍兼容旧协议的 5/10/20 与批次历史事件；旧设备不能改变已开始的计划。须随 V4 服务端及共享模型部署。
...
`GET /health`、同步快照新增 `learningVersion: 4`。`PUT /v1/sync` 可同时提交最多 200 条 `learningEvents`，回应 `ackedLearningEventIds` 与 `learningState`。原有客户端字段继续兼容。

SQLite 自动增加 `learning_events` 表，按 `(user_id, event_id)` 唯一去重。事件按发生时间和 ID 重放，支持离线晚到、自评、复习阶段、每日计划及完成事件；同日最早计划作为主计划，相同任务完成跨计划合并，其余真实学习记录仍保留。快照保留近 30 日任务及近 7 日计算需要的明细，服务端事件日志保留用于重放。此版本采用简单全量重放，当前本人及朋友规模适用；用户规模扩大前应增加快照检查点和符合最终隐私协议的保留策略。

服务端通过相对路径复用 `miniprogram/utils/learningModel.js`，因此部署时必须保留仓库目录结构，并同时更新该纯模型文件和 `server/src`，不能只复制 server 目录。服务端不会向小程序传输静态题库。

发布顺序：先用 SQLite 备份功能备份数据库，再部署兼容服务端并验证 health 的 V4 标识、现有用户登录、新学习状态往返及重复上传；最后发布小程序。回滚旧服务时保留新增表及数据库备份，客户端未确认事件不会丢弃。不要将生产环境变量或数据库放入仓库。
