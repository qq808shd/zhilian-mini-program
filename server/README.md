...
> 今日学习设置扩展：新增 `preferences` 事件（分类、可选成语批次、每天新学 5/10/20），计划携带设置快照。响应 `learningSettingsVersion: 1` 后客户端才确认新设置和对应计划；基础计划上限 35（复习 10、新学 20、配套题 5）。已开始的计划保持稳定，未开始的计划可按新设置替换。必须随 V4 服务端及共享模型一起部署。
